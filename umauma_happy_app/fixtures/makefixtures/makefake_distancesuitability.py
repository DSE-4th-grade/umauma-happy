...
def distancesuitability():
    ...
    ys = []  # json書き込み用配列に追加
    for i in range(len(StaticValue.distance_suitability_value)):
        date = datetime.datetime.now(jst).strftime("%Y-%m-%d %H:%M:%S")  # created_at & updated_at用
        fields = cl.OrderedDict()  # 格納するフィールドを定義
        fields["value"] = StaticValue.distance_suitability_value[i]
        fields["created_at"] = date
        fields["updated_at"] = date
        data = cl.OrderedDict()
        data["model"] = "umauma_happy_app.distancesuitability"  # 対象のmodelを設定
